...
public class ManutencaoProduto {
	public static void inserir(String nome, int estoque, float valorcompra, float promocao, float margem, int grupo,
			Connection conexao) throws NegocioException {
	...
	public static void alterar(int codigo, String nome, int estoque, float valorcompra, float promocao, float margem,
			float grupo, Connection conexao) throws NegocioException {
		PreparedStatement comando = null;
		try {
			comando = conexao.prepareStatement(
					"UPDATE produto SET nome = ?, estoque = ?,valorcompra = ?,promocao = ?,margemlucro = ?,grupo = ?) WHERE codigo = ?");
			comando.setString(1, nome);
			comando.setDouble(2, estoque);
			comando.setDouble(3, valorcompra);
			comando.setDouble(4, promocao);
	...
	public static void baixaEstoqueDefeito(int codigo, String motivo, Date data, int quantidade, Connection conexao)
			throws NegocioException {
		PreparedStatement comando = null;
		try {
			comando = conexao
					.prepareStatement("INSERT INTO registrobaixa (motivo,data,quatidade,produto) VALUES (?,?,?,?)");
	...
	public static ArrayList<Produto> listarProduto(Connection conexao) throws NegocioException {
		...
		try {
			comando = conexao.prepareStatement(
					"SELECT prod.codigo AS codigo_prod,  prod.nome as nome_prod,  prod.estoque as estoque_prod, "
	...
	public static String validarDados(String nome, int estoque, float compra, float promocao, float margem, int grupo) {
